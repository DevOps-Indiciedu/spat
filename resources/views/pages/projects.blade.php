        ...
        <div class="row">
            <div class="col-sm-12 col-lg-12">
                ...
                <div class="iq-card-header d-flex justify-content-between">
                    ...
                    <button type="button" class="btn btn-danger addProject" data-toggle="modal" data-target="#exampleModalCenteredScrollable">
                    {{ __('Add Project') }}
                    </button>
                </div>
                    ...
                    <table id="datatable" class="table table-bordered table-hover" >
                       <thead>
                           <tr>
                               ...
                               <th class="text-dark">{{ __('Lead Assessor') }}</th>
                               <th class="text-dark">{{ __('QA Assessor') }}</th>
                       ...
                       <tbody>
                        ...
                        @foreach ($projects as $data)
                           <tr id="row-{{ $data->id }}" class="text-center">
                               ...
                               <td>{!!  get_company($data->auditee_id)->company_name !!} ({!!  @get_location($data->auditee_location_id)->name !!})</td>
                               <td>{!!  get_assessor($data->auditor_company)->company_name !!} ({!!  @get_location($data->auditor_location_id)->name !!})</td>
                               <td>{!!  @get_user($data->auditor_id)->username !!}</td>
                               <td>{!!  @get_user($data->auditor_qa_id)->username !!}</td>
